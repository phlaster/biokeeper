...
docker run -d --network=dockernet --name python_server_test -v ./settings/db_settings.py:/db_settings.py:ro -v ./init.sh:/init.sh:ro -v ./sucsessful_requests.log:/logs.log -e GIT_TOKEN=$(cat token.txt) -p 1337:8080 ubuntu /bin/bash /init.sh;
